...
SidePanel.prototype =
{
    initialize: function(panelId, sizeElement, forceClose)
    {
        ...
        this.BodyField = $('bd').getElementsByClassName('bodyField')[0];

       
        this.panelWidth = this.panelId.getWidth();
        this.panelPoxX = this.panelId.offsetLeft;
        this.isVisible = true;
        this.isPinned = (Cookie.get(panelId) == 'pinned');
    ...
    {
        if (!this.isPinned)
            Cookie.set(this.panelId.id, 'pinned', 7);
        else
            Cookie.set(this.panelId.id, '', 7);
    ...
    close: function(e)
    {
		...
		this.BodyField.style.width = '1234px';
//            Effect.BlindUpFromRight(this.menuContent);
    ...
    open: function(e)
    {
        ...
        {
            this.menuContent.style.display = 'block';
		this.BodyField.style.width = '990px';
